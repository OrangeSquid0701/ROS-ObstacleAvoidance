...
class ObstacleDetector:
    def __init__(self):
        ...
        rospy.Subscriber('/realsense/points', PointCloud2, self.callback)
        
        # Track the last state of obstacle detection
        self.last_obstacle_state = None  

    def callback(self, point_cloud):
        # Define the obstacle detection range as 2 meter
        detection_range = 2.0
        ...
        if marker.points:
            rospy.loginfo("Obstacle detected within 2.0 meter!")
            self.marker_pub.publish(marker)
            self.obstacle_pub.publish(True)
        else:
            rospy.loginfo("No obstacles within 2.0 meter.")
